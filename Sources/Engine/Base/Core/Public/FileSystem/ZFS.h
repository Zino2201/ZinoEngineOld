 ...
 */
CORE_API TOwnerPtr<IFile> Read(const std::string_view& InPath,
	const EFileReadFlags& InReadFlags = EFileReadFlagBits::None);

/**
 * Create a file archive of the specified path for writing
 */
CORE_API TOwnerPtr<IFile> Write(const std::string_view& InPath, 
	const EFileWriteFlags& InWriteFlags = EFileWriteFlagBits::None); 

/**
 * Check if the specified file or directory exists
 */
CORE_API bool Exists(const std::string_view& InPath);

/** Check if the specified path is a directory */
CORE_API bool IsDirectory(const std::string_view& InPath);

/**
 * Iterate over all directories
 */
CORE_API bool IterateDirectories(const std::string_view& InPath,
	const TDirectoryIterator& InIt);

/**
 * Set the current write filesystem
 */
CORE_API void SetWriteFS(IFileSystem* InFS);

/**
 * Get a filesystem by name
 */
CORE_API IFileSystem* GetFileSystemByName(const std::string_view& InName);
 ...
 */
class CORE_API CFileSystemManager final : public CNonCopyable
{
public:
	struct SFSEntry
	{
