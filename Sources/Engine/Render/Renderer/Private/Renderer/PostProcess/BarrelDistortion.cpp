	...
	const RenderPassResourceID& InTarget)
{
	...
	static std::unique_ptr<CRSSampler> TEST_SAMPLER = nullptr;

	if(!TEST_SAMPLER)
		TEST_SAMPLER = std::unique_ptr<CRSSampler>(
			GRenderSystem->CreateSampler(SRSSamplerCreateInfo()));
	...
	return InFrameGraph.AddQuadRenderPass<SBarrelDistortionData>("BarrelDistortion",
		...
		{
			InContext->SetShaderTexture(0, 0, InFrameGraph.GetTexture(InColor));
			InContext->SetShaderSampler(0, 1, TEST_SAMPLER.get());
		},
