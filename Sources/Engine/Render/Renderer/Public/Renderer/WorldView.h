...
{	
	...
	std::unordered_map<ERenderPass, std::vector<size_t>> VisibleCachedDrawcalls;

	/** Visible meshes generated by dynamic proxies */
	std::vector<SMesh> VisibleDynamicMeshes;
	std::vector<ERenderPass> VisibleDynamicMeshRenderPassFlags;
	std::vector<const CRenderableComponentProxy*> VisibleDynamicMeshProxies;
	
	/** Render pass renderer for this view per pass */
	std::unordered_map<ERenderPass, CRenderPassRenderer> RenderPassRendererMap;

	SWorldView(CWorldProxy& InWorldProxy) : WorldProxy(InWorldProxy) 
	{
		VisibleCachedDrawcalls.reserve(static_cast<size_t>(ERenderPass::Count));
		RenderPassRendererMap.reserve(static_cast<size_t>(ERenderPass::Count));
	}
};
