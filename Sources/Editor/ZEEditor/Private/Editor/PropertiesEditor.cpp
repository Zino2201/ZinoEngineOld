	...
	: refl_class(in_class), object(in_object) 
{
	...
	{
		categories[property.has_metadata("Category") ? property.get_metadata("Category") : "Misc"].properties.emplace_back(&property);
	}
...
bool PropertiesEditor::draw()
{
	...
	for(auto& [name, category] : categories)
	{
		...
		{
			if(ImGui::BeginTable("PropertyTable", 2, ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_SizingStretchProp))
			{
